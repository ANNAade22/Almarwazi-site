...
const useAnimationLoop = (
  ...
  useEffect(() => {
    ...
    // Initialize offset when seqWidth is available
    if (seqWidth > 0) {
      // Normalize the current offset to prevent jumps
    ...
    const animate = (timestamp: number) => {
      ...
      // Always continue animation, even if seqWidth is 0
      if (seqWidth > 0) {
        // Continuously increment the offset without resetting
      ...
      // Always continue the animation loop - this ensures infinite scrolling
  ...
  ({
    ...
    const updateDimensions = useCallback(() => {
      ...
      if (sequenceWidth > 0) {
        ...
        // Ensure we have enough copies to fill the container plus extra for seamless looping
        const copiesNeeded =
          Math.ceil((containerWidth * 2) / sequenceWidth) +
          ANIMATION_CONFIG.COPY_HEADROOM;
        setCopyCount(Math.max(ANIMATION_CONFIG.MIN_COPIES, copiesNeeded));
      } else {
        // Fallback: set a minimum width to ensure animation works
        setSeqWidth(1000);
    ...
    // Separate effect to handle seqWidth updates without restarting animation
    useEffect(() => {
      if (seqWidth > 0) {
        const track = trackRef.current;
        if (track) {
